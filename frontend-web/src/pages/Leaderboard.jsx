...
function PodiumCard({ r, tall }) {
  ...
  return (
    <div
      ...
      style={
        tall
          ? {
              boxShadow:
                "var(--shadow-card), 0 0 40px -10px color-mix(in srgb, var(--yellow) 40%, transparent)",
    ...
    >
      ...
      <p className={`mt-3 truncate font-bold text-[var(--text)] ${tall ? "text-sm" : "text-xs"}`}>
        {r.username}
      </p>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e?.response?.data?.error || "Could not load leaderboard.");
      } finally {
