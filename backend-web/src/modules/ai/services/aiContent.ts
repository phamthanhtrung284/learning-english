...
const normalizeLesson = (data: any) => {
  const title =
  ...
  data.title = title;
  data.paragraphs = data.paragraphs.map((p: any) => ({
    text: p.text || "",
...
export async function generateLessonAI(topic: string, level = "C2") {
  ...
  try {
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You output only a single valid JSON object. No markdown, no commentary.",
        },
