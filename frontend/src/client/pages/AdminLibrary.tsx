...
function UsersTab() {
  ...
  const fetchUsers = useCallback(async (p = 0, q = "") => {
    ...
    try {
      ...
      setUsers((data as { users?: Record<string, unknown>[] }).users || []);
      setTotal((data as { total?: number }).total || 0);
      setPage(p);
    } catch (e) {
      setErr((e as { response?: { data?: { error?: string } } })?.response?.data?.error || "Load failed");
    } finally { setLoading(false); }
  }, []);

  useEffect(() => { queueMicrotask(() => void fetchUsers(0, "")); }, [fetchUsers]);

  const toggle = async (userId: string, field: string, current: boolean) => {
    setUpdating(userId + field);
    try {
      await api.patch(`/auth/admin/users/${userId}`, { [field]: !current });
      setUsers(u => u.map(x => String(x.id) === userId ? { ...x, [field]: !current } : x));
    } catch (e) {
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-[var(--border)]">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.map(u => (
                <tr key={String(u.id)} className="border-b border-[var(--border)] hover:bg-[var(--bg-soft)] transition">
                  <td className="px-4 py-3 font-semibold text-[var(--text)]">{String(u.username ?? "")}</td>
                  <td className="px-4 py-3 text-[var(--text-soft)]">{String(u.email ?? "")}</td>
                  <td className="px-4 py-3 text-[var(--text-soft)]">{new Date(String(u.createdAt ?? "")).toLocaleDateString("vi-VN")}</td>
                  <td className="px-4 py-3">
                    <button type="button" disabled={updating === u.id + "isPremium"}
                      onClick={() => toggle(String(u.id), "isPremium", Boolean(u.isPremium))}
                  ...
                  <td className="px-4 py-3">
                    <button type="button" disabled={updating === u.id + "isAdmin"}
                      onClick={() => toggle(String(u.id), "isAdmin", Boolean(u.isAdmin))}
...
export default function AdminLibrary() {
  ...
  const updateBlock = (index: number, val: string) => setBlocks(p => { const n = [...p]; n[index] = val; return n; });
  const deleteBlock = (index: number) => setBlocks(p => p.filter((_, i) => i !== index));
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Load failed");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const chaptersFlat = useMemo(() => {
    ...
    for (const s of series) {
      for (const ch of (s.chapters as Record<string, unknown>[]) || []) {
        rows.push({
  ...
  const loadChapterForEdit = async (id: string) => {
    ...
    try {
      ...
      setEditForm({
        ...
        label: "",
        authorLine: String(d.authorLine || ""),
        blurb: String(d.blurb || ""),
      });

      const loadedBlocks = ((d.paragraphs as { en?: string }[]) || []).map((p) => String(p?.en || "").trim()).filter(Boolean);
      ...
      if (fromList?.label) setEditForm((p) => ({ ...p, label: String(fromList.label) }));
    } catch (e) {
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Load chapter failed");
    } finally {
  ...
  const handleUpload = async () => {
    if (!file) { alert("Chọn file PDF trước đã."); return; }
    if (!form.seriesTitle.trim()) { alert("Nhập tên truyện (Series title)."); return; }
    if (!form.chapterTitle.trim()) { alert("Nhập Chapter title."); return; }
    ...
    try {
      ...
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Import failed");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Save failed");
    } finally {
      setEditLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Delete failed");
    }
  };

  const handleCoverUpload = async (seriesId: string, imageFile: File) => {
    if (!imageFile) return;
    ...
    try {
      ...
      setErr((e as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error || (e as Error)?.message || "Upload cover failed");
    } finally {
        ...
        <>
          ...
          <section className="surface-panel p-6 md:p-8">
            ...
            {loading ? (
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {series.map((s) => {
                  const BACKEND = typeof window !== "undefined" ? (process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000").replace("/api", "") || "http://localhost:4000" : "";
                  const coverSrc = s.coverImage ? `${BACKEND}${s.coverImage}` : null;
                  return (
                    <div key={String(s.id)} className="flex items-center gap-4 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-3">
                      <div className="h-20 w-14 shrink-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg-soft)]"
                        ...
                        {coverSrc ? (
                          ...
                          <div className={`flex h-full w-full items-center justify-center bg-gradient-to-br ${String(s.accent || "from-slate-600 to-slate-800")} text-2xl`}>
                            {String(s.coverEmoji || "📚")}
                          </div>
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-bold text-[var(--text)]">{String(s.displayTitle)}</p>
                        <label className="mt-2 block">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (f) handleCoverUpload(String(s.id), f);
                              e.target.value = "";
                            }}
                            disabled={coverUploading === String(s.id)}
                          />
                        </label>
                        {coverUploading === String(s.id) && (
          ...
          <section className="surface-panel p-6 md:p-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={fetchAll} className="glass-btn h-10 px-4 text-sm font-bold text-[var(--text)]">Refresh</button>
            </div>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-2">
                {chaptersFlat.map((ch) => (
                  <div key={String(ch.id)}
                    className={`rounded-2xl border bg-[var(--bg-card)] p-4 ${String(editingId) === String(ch.id) ? "border-[color-mix(in_srgb,var(--primary)_40%,var(--border))]" : "border-[var(--border)]"}`}
                  >
                    <div className="text-xs font-bold uppercase tracking-widest text-[var(--text-soft)]">{String(ch.seriesTitle)}</div>
                    <div className="mt-1 font-display text-base font-extrabold text-[var(--text)]">{String(ch.chapterTitle)}</div>
                    <div className="mt-1 text-sm text-[var(--text-soft)]">{String(ch.label ?? "")}</div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button type="button" onClick={() => loadChapterForEdit(String(ch.id))}
                        className="glass-btn h-10 px-4 text-sm font-bold text-[var(--text)]">Edit</button>
                      <button type="button" onClick={() => handleDelete(String(ch.id))}
                        className="glass-btn h-10 px-4 text-sm font-bold text-red-600 dark:text-red-300">Xóa</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section className="surface-panel p-6 md:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold text-[var(--text-soft)]">Tip: mỗi đoạn cách nhau 1 dòng trống.</p>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => setTab("manage")} className="glass-btn h-10 px-4 text-sm font-bold text-[var(--text)]">← Quay lại</button>
            </div>
          </div>

          {!editingId ? (
            <div className="mt-4 text-sm text-[var(--text-soft)]">Chưa chọn chapter. Quay lại tab Quản lý để chọn.</div>
          ) : editLoading ? (
            <div className="mt-4 text-sm text-[var(--text-soft)]">Đang tải…</div>
          ) : (
            <>
              <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                <input className="input-magic" placeholder="Chapter title" value={editForm.chapterTitle}
                  onChange={(e) => setEditForm((p) => ({ ...p, chapterTitle: e.target.value }))} />
                <input className="input-magic" placeholder="Label" value={editForm.label}
                  onChange={(e) => setEditForm((p) => ({ ...p, label: e.target.value }))} />
                <input className="input-magic" placeholder="Author line" value={editForm.authorLine}
                  onChange={(e) => setEditForm((p) => ({ ...p, authorLine: e.target.value }))} />
                <input className="input-magic" placeholder="Blurb" value={editForm.blurb}
              ...
              <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-2">
                <div className="flex gap-2">
                  <button type="button"
                    onClick={() => {
                      if (editorMode === "raw") {
                        ...
                        setBlocks(newBlocks); setBlockPage(0); setEditorMode("blocks");
                      }
                    }}
                  ...
                  <button type="button"
                    onClick={() => {
                      if (editorMode === "blocks") { setRawText(blocks.join("\n\n")); setEditorMode("raw"); }
                    }}
                ...
                {editorMode === "blocks" && (
                  <button type="button" onClick={cleanupBlocks} className="glass-btn h-9 px-4 text-xs font-bold text-amber-500">
                ...
                {editorMode === "raw" ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-2 px-1">
                      <div className="text-sm font-bold text-[var(--text-soft)]">Tổng cộng {blocks.length} blocks</div>
                      <div className="flex items-center gap-2">
                        <button type="button" disabled={blockPage === 0} onClick={() => setBlockPage(p => Math.max(0, p - 1))}
                          className="glass-btn h-8 px-3 text-sm disabled:opacity-30">Trang trước</button>
                        <span className="text-sm font-bold text-[var(--text)]">{blockPage + 1} / {Math.max(1, Math.ceil(blocks.length / BLOCKS_PER_PAGE))}</span>
                        <button type="button" disabled={blockPage >= Math.ceil(blocks.length / BLOCKS_PER_PAGE) - 1}
                          onClick={() => setBlockPage(p => p + 1)} className="glass-btn h-8 px-3 text-sm disabled:opacity-30">Trang sau</button>
                      </div>
                    </div>
                      ...
                      {blocks.slice(blockPage * BLOCKS_PER_PAGE, (blockPage + 1) * BLOCKS_PER_PAGE).map((block, i) => {
                        ...
                        return (
                          <div key={actualIndex} className="group relative flex gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-3 transition-colors hover:border-[color-mix(in_srgb,var(--primary)_30%,var(--border))]">
                            ...
                            <textarea className="w-full resize-y bg-transparent text-[15px] leading-relaxed text-[var(--text)] outline-none"
                              rows={Math.max(2, block.split("\n").length)}
                              value={block} onChange={(e) => updateBlock(actualIndex, e.target.value)} />
                          </div>
                        );
                      })}
                      {blocks.length === 0 && <div className="p-8 text-center text-[var(--text-soft)]">Không có nội dung.</div>}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="sticky bottom-4 mt-8 flex justify-end">
                <button type="button" disabled={editLoading} onClick={handleSaveEdit}
