...
import { GlossaryEntry, segmentLnParagraph } from "@share/utils/segmentLnParagraph";
import { splitParagraphToSentences } from "@share/utils/splitParagraphToSentences";
import { LN_COMMON_GLOSS } from "@share/data/lnCommonGloss";
import api from "@share/services/api";

const GRAMMAR_UNDERLINE: Record<string, string> = {
...
export default function LnChapterView({
  ...
  const glossary: Record<string, Omit<GlossaryEntry, "text">> = (chapter.glossary || {}) as Record<string, Omit<GlossaryEntry, "text">>;

  const [grammarByKey, setGrammarByKey] = useState<Record<string, { style?: string; role?: string }[]>>({});
  const [grammarLoadingKey, setGrammarLoadingKey] = useState<string | null>(null);
  const [grammarError, setGrammarError] = useState<string | null>(null);

  const [page, setPage] = useState(0);
  const PARAGRAPHS_PER_PAGE = 50;
  const totalPages = Math.ceil((chapter.paragraphs?.length || 0) / PARAGRAPHS_PER_PAGE);
  const currentParagraphs = (chapter.paragraphs || []).slice(
    page * PARAGRAPHS_PER_PAGE,
    (page + 1) * PARAGRAPHS_PER_PAGE
  );

  const runGrammar = useCallback(async (sentenceText: string, sentenceKey: string) => {
        ...
        {totalPages > 1 && (
          <div className="mt-16 flex flex-wrap items-center justify-center gap-4 border-t border-[var(--border)] pt-8">
            <button
              ...
              className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] h-12 px-6 text-[15px] font-bold text-[var(--text)] hover:border-[color-mix(in_srgb,var(--primary)_35%,transparent)] disabled:opacity-30"
            >
              ← Previous
            </button>
