...
const cleanJson = (text) => {
  try {
    ...
    console.error("translationCheck JSON parse:", e?.message);
    throw new Error("AI trả về JSON không hợp lệ.");
  }
};

/**
 * Chấm bản dịch tiếng Anh của người dùng so với câu gốc tiếng Việt.
 * Chuẩn TOEIC 900+ — không yêu cầu văn phong học thuật, tập trung vào
 * ngữ pháp chính xác, từ vựng tự nhiên, và cấu trúc câu rõ ràng.
 */
export const checkTranslationWithAI = async (vietnameseSentence, userTranslation) => {
  const prompt = `
...
`;
  ...
  return {
    ...
    // backward compat
    suggestions: Array.isArray(data.suggestedImprovements) ? data.suggestedImprovements.slice(0, 3) : [],
  };
};
